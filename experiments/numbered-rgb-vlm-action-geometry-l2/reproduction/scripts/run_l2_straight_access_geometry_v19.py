#!/usr/bin/env python3
"""Run the agreed straight-approach prompt on byte-identical v18 inputs."""
import argparse
import fcntl
import json
import shutil
...
SOURCE = ROOT / "experiments/vlm_action_geometry_single_info_l2_approach_access_geometry_v18"
DEST = ROOT / "experiments/vlm_action_geometry_single_info_l2_straight_access_geometry_v19"
VERSION = "l2_straight_access_geometry_v19"
SYSTEM = """Assess direct access to the target from the shelf opening.
...
def prepare():
    ...
    config = {
        **{k: reference[k] for k in ("scene_count", "scheduled_calls", "seeds", "run_order_seed",
        ...
        "schema": SCHEMA, "schema_sha256": reference["schema_sha256"],
        "gripper_information_supplied": False,
        "task_definition": "Straight approach from shelf opening without moving or contacting other objects; nearby objects block only when obstructing this approach.",
        "evaluation": "descriptive_predictions_and_paired_v18_changes_no_execution_ground_truth",
        "comparison_note": "Agreed system and user wording both changed; this is not an isolated ablation of the word straight.",
    }
    assert SCHEMA == reference["schema"]
    path = DEST / "config/experiment_config.json"
    if path.exists():
        assert engine.read(path) == config, "Frozen config changed"
        assert engine.read(DEST / "audit/preflight_report.json")["status"] == "pass"
        ...
        assert not (DEST / "logs/runs.jsonl").exists(), "Refuse to overwrite existing inference"
        ...
        schedule, copied = [], set()
        for prior in engine.rows(SOURCE / "config/run_table.jsonl"):
            row = dict(prior)
            assert prior["run_id"].startswith("v18__")
            row["run_id"] = prior["run_id"].replace("v18__", "v19__", 1)
        ...
        engine.write_json(DEST / "audit/preflight_report.json", {
            ...
            "unique_run_ids": len({r["run_id"] for r in schedule}), "scene_count": len(scenes),
            "condition_counts": dict(Counter(r["condition_key"] for r in schedule)),
            "identical_v18_images_geometry_model_settings_schema_seeds_order": True,
            "geometry_payloads_copied_without_changes": len(copied),
            "agreed_prompts_verbatim": True,
...
def main():
    ...
    with (DEST / ".runner.lock").open("a") as lock:
        ...
        if args.prepare_only:
            ...
            return
        engine.DEST, engine.VERSION, engine.SYSTEM = DEST, VERSION, SYSTEM
        records = engine.rows(DEST / "logs/runs.jsonl") if args.verify_only else engine.infer(config, schedule)
        assert all(r["schema_valid"] for r in records), "Invalid output schema"
        assert all(json.loads(r["raw_response"]) == r["parsed_response"] for r in records)
