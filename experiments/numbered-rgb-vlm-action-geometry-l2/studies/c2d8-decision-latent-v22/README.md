# C2 D8 결정 분기 latent cosine — 중간층 vs 끝단

## 무엇을 측정했나

- 대상: L2 v16의 C2 D8 25개 장면. 기존 5개 seed 응답 125개는 결과 라벨로만 연결했다.
...
- Raw cosine 300쌍 평균: block 24 **0.991748**, final **0.985997**.
- 중간–끝단 pairwise cosine 패턴 상관: **0.3751**.
- 끝단에서 pairwise cosine이 바뀐 절댓값 평균: **0.006814**.
- 같은 family와 다른 family의 평균 차이(같음−다름): block 24 **0.002485**, final **0.006074**.
- 같은 기대 결정과 다른 기대 결정의 평균 차이: block 24 **0.005032**, final **0.009687**.
- 최근접 장면이 같은 family인 경우: block 24 **12/25**, final **11/25**.
- 재추출 branch logit 방향과 기존 5회 다수결의 일치: **21/25**. 장면 설계 기대와의 일치: **20/25**.
...
```bash
.qwen3-vl/venv/bin/python scripts/extract_l2_c2d8_decision_latent_v22.py
.l1-latent/plot-venv/bin/python scripts/analyze_l2_c2d8_decision_latent_v22.py
```
