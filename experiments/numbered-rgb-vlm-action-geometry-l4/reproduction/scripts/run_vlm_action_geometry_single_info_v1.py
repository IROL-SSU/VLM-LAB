...
def build_user_prompt(row: dict, geometry: object, task_prompt: str) -> str:
    ...
    if "selected_blocking_reason" in row:
        task = task.replace(
            "{selected_blocking_reason}", str(row["selected_blocking_reason"])
        )
    if "selected_blocker_id" in row:
        blocker_text = (
            "null" if row["selected_blocker_id"] is None
            else str(int(row["selected_blocker_id"]))
        )
        task = task.replace("{selected_blocker_id}", blocker_text)
...
def semantic_validate(level: str, parsed: object, row: dict, mapping: dict) -> list[str]:
    ...
    if level == "L1":
        ...
        if row.get("action_only"):
            if set(parsed) != {"action"}:
                errors.append("action-only output must contain only action")
            if action not in {"RETRIEVE", "TRANSLATE", "ROTATE", "LIFT_AND_RELOCATE"}:
                errors.append("action is not one of the four allowed choices")
            return errors
        ...
        if "selected_blocking_reason" in row:
            selected_reason = row["selected_blocking_reason"]
            selected_blocker = row.get("selected_blocker_id")
            if selected_reason == "NONE":
                if action != "RETRIEVE" or object_id != target_id:
                    errors.append(
                        "L3 blocking_reason=NONE requires RETRIEVE of target_object_id"
                    )
            else:
                if action == "RETRIEVE":
                    errors.append("non-NONE L3 blocker selection forbids RETRIEVE")
                if object_id != selected_blocker:
                    errors.append("object_id does not equal the L3-selected blocker_id")
        if action == "TRANSLATE":
                ...
                try:
                    ...
                    if row.get("action_only"):
                        ...
                        scoring = {
                            ...
                            "action_matches_advisory": action == advisory,
                            "task_correct": None,
                            "evaluation": "action_label_only_no_isaac_sim_replay",
                        }
                    else:
